...
function getTitleFromResult(result) {
  let title;
  const nameMatch = result.name.match(/(.+?) - Wikipedia$/);
...
async function getRelatedImages(q, geo) {
  const response = await images.search({ mkt: geo.market, q });
...
async function handleResultMatch({ result, geo }) {
  ...
  await Promise.all([
    ...
    (async () => (relatedImages = title ? await getRelatedImages(title, geo) : null))()
  ]);
