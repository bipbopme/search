...
import { urlJoin, convertToNumber, getInsertIndex } from '../../utils';
...
async function handleUserResultMatch({ query, index, patternMatch }) {
  return handleResultMatch({ query: patternMatch[1], index });
}
...
export default {
  ...
  matches: [
    {
      result: {
        urlPattern: /youtube\.com\/user\/(.*?)(\/|$)/,
        maxIndex: 9,
        onMatch: handleUserResultMatch
      }
    },
    {
      result: {
