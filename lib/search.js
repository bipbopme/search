...
export async function search(params, location = null) {
  ...
  if (timeRemaining > 0) {
    await Promise.race([delay(timeRemaining), enhanceResponse(bestQuery, response, location)]);
  }
  ...
  console.log('Enhancement allotment:', `${timeRemaining}ms`);
  console.log('Enhancement time:', `${endTime - searchEndTime}ms`);
