...
async function getEnhancement(query, response, enhancer) {
  ...
  if (!enhancement) {
    enhancement = await getResultEnhancement(query, response, enhancer);
  }
...
async function getResultEnhancement(query, response, enhancer) {
  ...
  if (resultMatcher?.pattern && enhancer.onResultMatch) {
    ...
    for (let i = 0; i <= maxIndex; i++) {
      ...
      if (match) {
        enhancement = await enhancer.onResultMatch({ query, result, index: i, match });
        break;
      }
    }
  }
