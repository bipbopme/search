...
export async function enhanceResponse(query, response, location) {
  const answers = [];
  const supplements = [];
  ...
  await Promise.all(
    enhancers.map(async enhancer => {
      const enhancement = await getEnhancement(query, intent, location, response, enhancer);
...
async function getEnhancement(query, intent, location, response, enhancer) {
  ...
  for (let i = 0; i < enhancer.matches.length && !enhancement; i++) {
    ...
    if (match.query) {
      enhancement = await getQueryEnhancement(query, location, match, enhancer);
    } else if (match.result) {
      enhancement = await getResultEnhancement(query, location, match, enhancer, response);
    } else if (match.intent) {
      enhancement = await getIntentEnhancement(query, location, intent, match, enhancer);
    } else {
...
async function getQueryEnhancement(query, location, match, enhancer) {
  ...
  if (queryMatcher.pattern && queryMatcher.onMatch) {
    ...
    if (patternMatch) {
      ...
      enhancement = await queryMatcher.onMatch({ query, location, patternMatch });
    }
  }
...
async function getResultEnhancement(query, location, match, enhancer, response) {
...
async function getIntentEnhancement(query, location, intent, match, enhancer) {
  ...
  if (intentMatcher.name === intent.intent && intentMatcher.onMatch) {
    ...
    enhancement = await intentMatcher.onMatch({ query, location, intent });
  }
